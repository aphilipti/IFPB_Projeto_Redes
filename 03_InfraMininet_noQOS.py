#!/usr/bin/env python3
"""
Topologia Mininet SEM QoS - Versão para comparação
"""
import argparse
import os
import time
...
def setup_ssh_access(routers):
    ...
    for r in routers:
        ...
        r.cmd('echo "mininet:mininet" | chpasswd')
        r.cmd('/usr/sbin/sshd -D &')
        info(f'  SSH ativado em {r.name}\n')

def build_net_no_qos(bw, delay, loss):
    ...
    info('*** ATENÇÃO: QoS DESABILITADO nesta execução!\n')
    info('*** Configuração de rede completa SEM QoS.\n')
...
if __name__ == '__main__':
    setLogLevel('info')
    parser = argparse.ArgumentParser(description='Topologia Mininet SEM QoS')
    ...
    build_net_no_qos(args.bw, args.delay, args.loss)